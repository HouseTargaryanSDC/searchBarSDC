            ...
            <div className={style.searchedSection}>
                {cityMatch.length !== 0 ? <div className={style.locationResults}>
                    ...
                    <ul className={style.ulContainerSearch}>
                        {props.cityMatch.map(locaMatch => <div className={style.hoverOnSearch} onMouseEnter={props.handleHover}><div className={style.searchResultText}>{locaMatch}</div></div>)}
                    </ul>
                </div> : <div></div>}
            </div>
            <div className={style.searchedSection}>
                {cuisineMatch.length !== 0 ? <div className={style.cuisinesResults}>
                    ...
                    <ul className={style.ulContainerSearch}>
                        {props.cuisineMatch.map(cuisMatch => <div className={style.hoverOnSearch} onMouseEnter={props.handleHover}><div className={style.searchResultText}>{cuisMatch}</div></div>)}
                    </ul>
                </div> : <div></div>}
            </div>
            <div className={style.searchedSection}>
                {restaurantMatch.length !== 0 ? <div className={style.restaurantResults}>
                    ...
                    <ul className={style.ulContainerSearch}>
                        {props.restaurantMatch.map(restMatch => <div className={style.hoverOnSearch} onMouseEnter={props.handleHover}><div className={style.searchResultText}>{restMatch}</div></div>)}
                    </ul>
                </div> : <div></div>}
            </div>
        </div>
